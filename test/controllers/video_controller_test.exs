defmodule Alegro.VideoControllerTest do
  use Alegro.ConnCase
  ...
  test "lists all user's videos on index", %{conn: conn, user: user} do
    user_video = insert(:video, title: "funny cats", user: user)
    other_video = insert(:video, title: "another video", user: insert(:user, username: "other"))
  ...
  test "creates user video and redirects", %{conn: conn, user: user} do
    ...
    assert Repo.get_by!(Alegro.Video, @valid_attrs).user_id == user.id
  end

  @tag login_as: "max"
  test "does not create video and renders errors when invalid", %{conn: conn} do
    count_before = video_count(Alegro.Video)
